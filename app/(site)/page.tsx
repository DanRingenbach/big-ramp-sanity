...
export default async function Home() {
  ...
  return (
    <main className="max-w-7xl mx-auto lg:px-16 px-6">
      ...
      <section className="pb-20 grid lg:grid-cols-2 grid-cols-1sm:flex sm:flex-col-reverse gap-x-6 justify-items-center">
        <CurrentExhibition />
        <a href={`/exhibitions/${singleJob.slug}`}><div className=" p-5 order-2 lg:order-none hover:shadow-lg">
      ...
      <h1 className="text-3xl font-bold tracking-tight sm:text-5xl mb-6 lg:leading-[3.7rem] leading-tight">Recent News</h1>

      <section className="grid lg:grid-cols-1 grid-cols-1sm:flex sm:flex-col-reverse gap-x-6 justify-items-center ">
        <div className="hover:shadow-lg">
          <Link
            href={`/posts/${singlePost.slug}`}
            key={singlePost._id}
            className="flex flex-col  gap-x-4  ease-in-out "
          >
